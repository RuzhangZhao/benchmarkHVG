...
def myscCAD(data,
    ...
    if total2000:
      ...
      total = len(sg)
      while total >= 2001:
        print(">>> currently there are %d genes in total" % total)
        k = k - int((total-1999) / 2)
        
        print(">>> Identification of top %d HVGs is currently in progress..." % k)
        sc.pp.highly_variable_genes(adata, n_top_genes=k)
        hvgs = list(adata[:, adata.var.highly_variable].var_names)
        
        rfgs = list(adata.var_names[np.argsort(-gene_imp)[:k]])
    ...
    return result, overlap, rename_comb_subclusters, remain_degs_list, sg, all_gene_names, all_gene_imp
...
def getHIG(data,
    ...
    selected_gene_imp = gene_imp

    return selected_gene_names, selected_gene_imp
...
def getCADlist(data,
    ...
    return sg, all_gene_names, all_gene_imp
