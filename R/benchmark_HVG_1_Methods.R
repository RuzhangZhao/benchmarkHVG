...
hvg_pca<-function(rna_mat,
    ...
    seurat.obj12<-FindVariableFeaturesMix(seurat.obj12,method.names = "disp_PFlogPF",nfeatures = nfeatures)
    ...
    #################################################
    #14.cell ranger selection (dispersion based, scanpy):
    message("Method14: cell ranger")
    library(reticulate)
    sc = import("scanpy")
    seurat.obj14<-CreateSeuratObject(rna_mat,verbose = F)
    scanpy_rna=sc$AnnData( t((seurat.obj14@assays$RNA@counts)))
    ...
    VariableFeatures(seurat.obj14)<-top_scanpy_cell_ranger
    ...
    #################################################
    #15.highly expressed genes
    message("Method15: mean_max_ct")
    seurat.obj15<-CreateSeuratObject(rna_mat,verbose = F)
    row_means_count<-rowMeans(seurat.obj15@assays$RNA@counts)
    hvg_rank<-savis_nth(row_means_count,nfeatures)
    VariableFeatures(seurat.obj15)<-rownames(seurat.obj15@assays$RNA@counts)[hvg_rank]
    ...
    #################################################
    #16.highly expressed genes normalized count
    message("Method16: mean_max_nc")
    seurat.obj16<-CreateSeuratObject(rna_mat,verbose = F)
    seurat.obj16<-NormalizeData(seurat.obj16,verbose = F)
    seurat.obj16@assays$RNA@data@x<-exp(seurat.obj16@assays$RNA@data@x)-1
    row_means_count<-rowMeans(seurat.obj16@assays$RNA@data)
    hvg_rank2<-savis_nth(row_means_count,nfeatures)
    VariableFeatures(seurat.obj16)<-rownames(seurat.obj16@assays$RNA@counts)[hvg_rank2]
    ...
    #################################################
    #17.highly expressed genes lognormalized count
    message("Method17: mean_max_lognc")
    seurat.obj17<-CreateSeuratObject(rna_mat,verbose = F)
    seurat.obj17<-NormalizeData(seurat.obj17,verbose = F)
    row_means_count<-rowMeans(seurat.obj17@assays$RNA@data)
    hvg_rank3<-savis_nth(row_means_count,nfeatures)
    VariableFeatures(seurat.obj17)<-rownames(seurat.obj17@assays$RNA@counts)[hvg_rank3]
    ...
    #################################################
    #18.Seurat SCT
    message("Method18: SCT")
    seurat.obj18<-CreateSeuratObject(rna_mat,verbose = F)
    seurat.obj18<-SCTransform(seurat.obj18,variable.features.n=2000,verbose = F)
